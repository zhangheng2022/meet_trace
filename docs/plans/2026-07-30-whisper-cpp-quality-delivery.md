...
> 状态：执行中（阶段 0～4 工程实现及 Android 外部语料执行链完成；真实语料质量门槛阻断）
...
- 自动发布评估输入同步升级为 schema 7，只有绑定人工复核证明的 `product-meeting`、有效 manifest
...
- 新入口已在 API 36 x86_64 模拟器完成编译、安装与缺参跳过检查；Small 固定权重也已
  ...
  静音冒烟，但只保留为评测候选。正式三类产品会议矩阵尚未提供，因此 Hard Gate 0
  仍为 `blocked`，生产参数不切换。
...
| 阶段 | 状态 | 进入条件 | 退出证据 |
|---|---|---|---|
| 0 边界与基线 | 工程完成 / 人工复核阻断 | 隔离 worktree | Step 21、可恢复 Android 质量链和 90 段私有冗余候选复核包已完成；正式人工标签缺失 |
| 1 Android 模拟器与 PCM | 通过 | 阶段 0 工程基线 | x86_64、30 秒 PCM、100 次事实录音、10 次完整会议、最终快照 |
| 2 解码 Profile | 工程完成 / 候选未激活 | 阶段 1 | ABI/Profile、可恢复矩阵与 100 次生命周期已完成；正式人工语料参数结果缺失 |
