...
> 状态：阶段 0～4 工程硬门槛通过；真实产品会议质量为非阻断 `not_tested`
> 当前质量结论：Small Baseline 固定窗口在 API 36 x86_64 模拟器的合成噪声矩阵中
> 完成 4/20 段后推理失败，候选保持 No-Go；不外推为 Android 真机结论。
> 基线分支：`dev`
> 计划日期：2026-07-30
...
1. 定义正式质量 manifest：不少于 20 段纯静音、20 段纯噪声和 20 段带关键事实的
   会议语音；语音同时覆盖起始和结束边界。manifest 只保存：
   - corpus ID；
   - 去敏状态；
   - 证据类别（产品会议、公开回归或合成烟测）；
   - 来源和许可标识；
...
3. 质量脚本必须输出 JSON/CSV：
   ...
   - 静音/噪声幻觉；
   - 峰值 RSS、能耗和温控引用。
4. 用现有固定 2 秒窗口分别跑 Base/Small，作为后续唯一对照组；VAD 阶段在同设备、
   模型、Profile 和语料上增加独立 `vad-segmented-v1` 管线，不覆盖基线观测。
...
- 产品边界已批准。
- Android 模拟器工程证据可追溯且未提交音频。
- 确定性静音、事实 PCM、模型锁定和生命周期不变量通过。
- 发布评估器能区分工程硬门槛与非阻断产品质量观察。
...
- 语料契约已升级为 schema 2，正式入口默认只接受 `product-meeting`；公开回归和合成
  烟测必须显式选择，且证据类别与来源会进入报告。
- 原始观测与报告已升级为 schema 4，固定窗口、生产 VAD 和召回候选具有独立
  `pipelineId`；除同条件噪声幻觉下降率和关键事实召回变化外，还强制记录 ASR 调用
  次数、VAD 语音段数、零检测样本数和语音覆盖率。
- 自动发布评估输入同步升级为 schema 8，产品会议语料、人工证明、召回、噪声幻觉、
  Preview P95 和语音首尾事实是非阻断观察项；无证据时输出 `not_tested`。阶段 0～4
  生命周期和最终快照不变量继续阻断。报告 schema 4 增加 `blocking` 和分类汇总；
  阶段 0～4 报告不会混入
  后续 Android 真机、iOS 和 Release 门槛。Android 模拟器证据由完整通过日志自动
  推导并绑定 SHA-256。质量聚合报告同样绑定 SHA-256，并由专用适配器逐字段写入
  corpus、双模型、三 Profile、三 Pipeline、VAD 和 Preview 门禁；CLI 校验仓库边界、
  实际哈希、隐私字段和输入一致性。旧输入保持完整 Alpha 发布评估语义。
...
阶段 0～4 工程报告为 `go` 且 OCR 无 Critical/High 后允许集成到 `dev`。这不代表
阶段 5～7 或 Alpha 发布完成；产品会议质量观察项为 `not_tested` 时不得宣传质量结论。

## 4. 阶段状态表

| 阶段 | 状态 | 进入条件 | 退出证据 |
|---|---|---|---|
| 0 边界与基线 | 通过 | 隔离 worktree | PRD V0.8、schema 8/4 评估器、Android 证据与非阻断观察口径 |
| 1 Android 模拟器与 PCM | 通过 | 阶段 0 工程基线 | x86_64、30 秒 PCM、100 次事实录音、10 次完整会议、最终快照 |
| 2 解码 Profile | 通过 / 候选未激活 | 阶段 1 | ABI/Profile、可恢复矩阵与 100 次生命周期已完成；产品质量结果 `not_tested` |
| 3 官方 VAD | 通过 | 阶段 2 工程能力 | 静音/chunk/模拟器 100 次生命周期通过；真实噪声结果 `not_tested` |
| 4 预览与最终 | 通过 | 阶段 3 工程能力 | 确定性最终快照和故障注入通过；真实关键事实结果 `not_tested` |
