...
4. `Alpha Release Reconciler` 立即运行一次，之后于每小时第 7、22、37、52 分钟查询 App Store Connect 和 Microsoft Store，并幂等提交、复用或恢复同一候选的固定 Package Flight。
5. TestFlight 同一 build 审核通过并进入 `Testing`，且 Windows Flight 达到 `Published`、精确包身份与候选一致后，协调器把同一 MSIX 提交 100% non-flighted production submission。
6. 正式 submission 达到 `Published/Public` 且精确包身份一致后，协调器生成 schema 3 发布门禁；Android 不重复验证。Windows 门禁不依赖专用机，也不证明 Store 客户端安装、启动、更新或卸载。
7. 全部不可变回执通过后，协调器以内部输入恢复 `Alpha Release`。最终 job 自动公开原 Draft，从公开地址重新下载 Android APK 并核对 SHA-256，成功后才原子前移签名更新指针。
...
flowchart LR
  ...
  C --> E{每小时 7/22/37/52 分协调}
  D --> E
  E --> F[Android 已验证 + Flight Published 精确回执]
  F --> G[同一 MSIX production 100% 提交]
  G --> H[Published / Public]
  H --> I[production 精确包回执]
  I --> J[schema 3 门禁并自动公开 Draft]
  J --> K[公开 APK 摘要复核]
  K --> L[原子更新签名指针]
```

正常路径没有 `github-release` required reviewer，也没有最终审批评论。Environment 仍用于隔离 Secret 和限制来源分支。
...
| 公开门禁 | 单次 Android Firebase ARM、TestFlight Testing、Flight Published 与 production Published/Public 精确包回执全部匹配同一候选；Windows 不含客户端生命周期证明 |
...
脚本先校验关键 Secret/Variable 并设置固定 Variable，所有预检成功后才清除五个发布 Environment 的 wait timer 和 required reviewer，同时保留分支部署策略。脚本不读取、复制或输出 Secret；缺失 Secret 必须用 `gh secret set --env <environment> <name>` 单独配置。
...
阻断时 Draft、公开 Release、tag 和旧 `updates/alpha/alpha.json` 均保持不变。新阻断会重新打开已关闭的同名 Issue；外部状态恢复或进入正常等待时自动关闭对应 Issue，下一次协调继续复用原候选。
...
2. Reconciler 以 Partner Center API 分别生成 Flight `Published` 和 production `Published/Public` 脱敏回执，逐字段核对 product/flight/submission、候选文件名、`1.0.<build>.0`、x64 与上传状态。
3. production 提交前必须重新下载来源运行的不可变 MSIX 并核对 SHA-256，再以 100% non-flighted 方式提交同一文件。schema 3 门禁包含两个 Store API 回执和 `validations.android`，不包含 Windows 客户端生命周期回执。
4. GitHub 托管 Windows Server 的静态包审计、Store 认证与 API 状态不能证明真实客户端安装、启动、更新或卸载；发布说明和验收结论不得声称已验证这些行为。公开 Draft 后不再启动独立审计工作流；最终发布 job 直接从公开地址重下 Android APK 并复核摘要。

## 6. 恢复、修复与撤回

- 候选构建失败：修复配置或工作流后，以相同 Draft `release_id` 重跑；只有身份匹配的成功候选可复用。
- 外部审核等待：不要重跑。协调器按每小时四次的错峰计划自动继续。
