...
- 仓库当前未包含 Silero VAD ONNX 权重。按照仓库“下载模型不得直接提交”的规则，需要用户明确批准后，才能选择官方 `silero_vad.int8.onnx` 或 `silero_vad.onnx`，补充固定来源、字节数、SHA-256、Manifest 和 NOTICE，并纳入 APK。
- 权重就位后必须在 Android 设备验证真实 VAD 初始化、连续 PCM 输入、尾段 flush、资源释放、积压恢复和录音完整性。
