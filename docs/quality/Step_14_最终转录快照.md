...
- Factory 仍只按明确 ID/版本创建一个 Engine，不自动回退或混用标准/高级模型。
...
4. 详情页自动使用本场锁定的标准 Paraformer 处理完整事实音频，显示来源模型和两个带时间戳最终片段。
5. 页面显示“重新转录会生成独立快照；成功后才替换当前结果”，只列出模拟器当前已安装的标准模型；同模型重转录可触发并返回完成态。
6. 事实音频和 SQLite 数据库继续保存在应用私有目录，进程持续存活，无 AndroidRuntime 崩溃。

## 已知风险与下一步

- 模拟器麦克风输入不构成准确率评测语料；Step 18 仍需在相同会议语料与实体设备上比较双模型 RTF、延迟、内存、能耗、温控和关键事实召回。
- 高级模型未安装在本次模拟器数据中，因此跨模型重转录由单元/组件测试覆盖；Qwen 真机性能与许可仍沿用 Step 01/10 的发布门槛。
