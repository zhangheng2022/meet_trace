...
> 状态：活动；固定 TestFlight 外测审核、Windows Package Flight/production 精确 API 回执和统一自动公开门禁已实现；schema 3 门禁首次成功运行前，Windows 继续标记为“规划中/未就绪”
>
> 更新日期：2026-08-26
>
> 上游：[Alpha PRD V1.5](../product/Alpha_PRD_无登录版.md)
...
- Windows x64 Debug/Release 工程、SQLite FFI、输入设备锁定/一次回退、连续性事件、单实例激活、录音 close 转托盘、“停止并退出”安全封存及睡眠/恢复缺口记录链已实现并有自动化/本机构建冒烟。常规 CI 保留不可分发开发探针；正式 `Alpha Release` 生成固定 Partner Center 身份的 Store MSIX，只把包体上传 Actions Artifact 和 Partner Center。协调器以 Store CLI/API 核验 Flight 与 production 的精确包身份和状态；自动更新解析器只接受 Store ID `9PHHSJMWK06G` 和包身份 `zhangheng2026.MeetTrace`。该门禁不证明 Store 客户端生命周期；schema 3 尚无成功生产运行，因此 Windows 当前必须继续标记为“规划中/未就绪”。
- PRD V1.5 要求 Android/iOS/Windows 同 SHA 的构建、自动化、分发与统一公开门禁；发布链只保留两个工作流，不要求专用 Windows 验证机或目标设备人工验收记录。
...
| 候选协调 | `.github/workflows/alpha-release-reconcile.yml` | Flight 提交/恢复、TestFlight/Store 错峰轮询、Windows Flight/production 精确 API 回执、100% production 与 schema 3 最终门禁 |
| Windows 发布门禁 | `quality.yml` 开发探针；两个发布工作流的静态包审计与 Store API 状态 | 固定 Store 身份、三平台同 SHA/版本、Flight Published 和 production Published/Public 精确回执形成机器合同；不证明客户端生命周期，首次 schema 3 生产运行前不得宣称闭环 |
...
| Windows | Windows 10 22H2/11 x64、固定 Store 身份 | 规划中/未就绪；要求 MSIX 静态审计、Flight `Published`、production `Published/Public` 精确包回执与 schema 3 统一门禁，不声明真实 Store 客户端生命周期已验证 |
...
最终公开由协调器生成的不可变门禁决定，操作见[发布流程](../project/GitHub_版本发布流程.md)。schema 3 门禁逐项核对同一候选身份、TestFlight `APPROVED/Testing`、Store Flight `Published`、production `Published/Public` 及两阶段精确包回执；任一不匹配都会阻断公开并维护 `release-blocked` Issue，不依赖 `github-release` 人工审批或专用 Windows runner。
