...
> 状态：活动；Android、iOS 与 Windows 已完成首次三平台统一公开，Windows 已使用固定 Store 身份通过限定受众、正式认证和受保护人工证明门禁，签名更新指针已前移；Store 安装、卸载和更新纵向自动化仍待闭环
>
> 更新日期：2026-08-21
>
...
| Windows 发布门禁 | `quality.yml` 开发探针；`alpha-release.yml` Store 候选与公开状态核验 | 固定 Store 身份、三平台同 SHA/版本、受保护人工证明或可选 API 确认正式 submission Published/Public、同版本唯一 x64 包与模式化脱敏回执；限定受众、首次认证和统一公开已完成，待闭环 Store 安装、卸载与内置更新纵向自动化 |
...
| Windows | Windows 10 22H2/11 x64、固定 Store 身份 | MSIX 审计、Private audience/Flight、正式认证与统一公开已通过；安装、卸载与 Store 更新纵向自动化仍待闭环 |
