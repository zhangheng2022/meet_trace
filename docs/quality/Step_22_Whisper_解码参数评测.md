...
> 状态：工程能力完成，真实产品会议评测为非阻断 `not_tested`
> 日期：2026-07-30
...
即使 VAD 完全拦截合成噪声，也不能冒充产品会议的噪声幻觉观察结论。
...
| Base 关键事实召回不低于阶段 0 | `not_tested`：非阻断产品质量观察 |
| Small 关键事实召回不低于 Base | `not_tested`：非阻断产品质量观察；当前模拟器的合成噪声固定窗口另有推理失败，候选不得激活 |
| Small 固定窗口模拟器稳定性 | `No-Go`：20 段合成噪声只完成 4 段，第 5 段前推理失败；保留私有日志与部分观测 |
| Preview 句后延迟 P95 ≤ 3 秒 | `not_tested`：非阻断产品质量观察 |
| 100 次 context 生命周期无持续增长 | 通过：第 10→100 次 RSS +6,242,304 bytes，小于 32 MiB 上限 |

产品会议观察项未执行时不得声称质量通过。Small 合成矩阵的已知失败仍禁止激活对应
候选 Profile；该候选激活约束独立于发布阻断决策。
