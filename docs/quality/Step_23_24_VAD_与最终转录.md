...
> 状态：阶段 3～4 工程硬门槛通过；真实产品会议质量为非阻断 `not_tested`
> 日期：2026-07-30
...
- 实时预览固定使用 threshold 0.35、min speech 100 ms、pad 100 ms；持续语音每 3 秒
  产出一个稳定子区间，避免等待整段讲话结束。
- 每次刷新使用最多 10 秒滚动上下文，相邻窗口按最长后缀/前缀只发布新增文本；
  滚动缓冲有上限，异常长语音的单次 ASR 输入仍不得超过 15 秒。
- Preview 使用 temperature increment 0.2；Native 暂时固定 `audio_ctx=0`，质量稳定前
  不启用缩短编码器上下文的性能优化。
...
- 最终转录固定使用普通 VAD 默认值 threshold 0.5、min speech 250 ms、pad 30 ms，
  不继承实时预览的召回参数。
...
产品会议观察结果只接受 `product-meeting` 证据。合成静音可关闭确定性零误检工程
门槛，但不得替代真实噪声、远场和语音首尾结果；后者缺失时保持非阻断
`not_tested`。
...
远场和语音首尾。小米实机会中诊断进一步确认默认参数导致实时文字长时间不出现，因此
实时预览采用召回参数；最终转录仍使用官方默认参数。质量研究继续把固定窗口、默认 VAD
和召回 VAD 放在同一报告中比较，两者不得被误写为同一生产管线。
...
| 会中预览与最终转录参数独立、显式且不会互相泄漏 | 通过 |
| 预览丢弃不改变最终完整 PCM 重跑 | 通过 |
| 重试和重新转录只能使用本场锁定模型/版本 | 通过 |
| 时间戳、排序、旧快照原子保留、总结事实源 | 通过 |
| 噪声幻觉相对阶段 0 下降至少 80% | `not_tested`：非阻断产品质量观察 |
| 语音首尾关键事实不丢失 | `not_tested`：非阻断产品质量观察 |
| VAD 100 次生命周期无持续泄漏 | 通过 |

工程硬门槛允许进入集成和后续发布阶段；未执行的产品会议观察项不得声明质量通过。
