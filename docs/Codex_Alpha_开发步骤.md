...
- 已有 184 个应用壳、组件、Step 01、领域层、本地存储、模型生命周期、可靠录音、官方 ASR adapter、双 Engine、Factory、VAD 和会议主链自动化测试。
...
Step 00、Step 02～13 已完成；Step 01 的代码与 Mi 10 技术验证已完成，但外部语料、低端设备和公开分发许可闭环仍在进行中。当前前半段会议主链已接通；会议结束后只进入 `processing`，最终转录快照仍属于 Step 14，不得把处理详情占位状态误写为最终转录已经完成。
