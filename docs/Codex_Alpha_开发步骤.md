...
- 已有 212 个应用壳、组件、Step 01、领域层、本地存储、模型生命周期、可靠录音、官方 ASR adapter、双 Engine、Factory、VAD、会议主链、最终转录和说话人降级自动化测试。
...
Step 00、Step 02～15 已完成；Step 01 的代码与 Mi 10 技术验证已完成，但外部语料、低端设备和公开分发许可闭环仍在进行中。会议结束后会从完整事实音频生成并激活最终转录快照；说话人增强失败不会改变快照完成态，AI 总结仍属于 Step 16，不得读取会中临时文本或未激活快照。
