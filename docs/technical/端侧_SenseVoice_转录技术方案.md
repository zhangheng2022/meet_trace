...
发布工作流仅从不设 reviewer 的 `github-release` Environment 读取 Ed25519 私钥 seed。协调器先验证 TestFlight、Store Flight/production 精确包回执和单次 Android 分发回执，再恢复最终 job；最终 job 验证 schema 3 不可变门禁、公开原 Draft，并用 Contents API 的旧 blob SHA 原子更新 `updates/alpha`。Windows 门禁不证明 Store 客户端生命周期。修复允许同 release/build 重签，撤回只允许当前 `publicApproved` 变为 `withdrawn`，已撤回版本不能重新公开。密钥不进入命令行、日志、Artifact、Release 或生成分支。
...
- Windows 只构建 x64 MSIX，当前唯一正式路线为 Microsoft Store。Manifest 固定 Store 的 Name、Publisher、PublisherDisplayName，Store 包版本使用 `1.0.<共享发布构建号>.0` 传输映射并另行记录营销版本；候选从可验证 CI 进入 Actions Artifact。一次性 bootstrap 完成 Partner Center 产品、固定 Flight 与自动认证后发布配置；每个候选自动进入 Package Flight，取得 `Published` 精确包回执后将同一包提交 100% production，再等待 `Published/Public` 精确包回执。该门禁不依赖专用机，不证明 Store 客户端安装、启动、更新或卸载。Store 签名前的候选不得放入 GitHub Release 或引导用户旁加载。
