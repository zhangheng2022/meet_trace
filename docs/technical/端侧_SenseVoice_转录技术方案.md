...
> 上游：[Alpha PRD V1.9](../product/Alpha_PRD_无登录版.md)。本文只定义实现契约，不重复产品验收或可直接读取的 Manifest 字段。

## 1. 不变量

- 事实 PCM 是唯一事实源；先落盘，再把副本送给 VAD、ASR、波形和说话人分离。
- 录音不依赖推理。预览队列可丢最旧任务，录音字节不可丢。
- 本地 ASR 只通过官方 `sherpa_onnx` 包接入固定 SenseVoice；在线 ASR 通过明确协议适配同一 `AsrEngine`；说话人分离只通过官方 `OfflineSpeakerDiarization` API。
- 权重不进入 APK、IPA 或 MSIX；本地能力在其资源就绪后可用，首页、历史与在线配置不依赖本地权重。
- 会议创建时锁定 ASR 来源、端点、语言、ITN 和分离配置；会中不得换配置，同源重试读任务快照；会后手动换来源创建新任务和新快照。
...
在线来源持久化非秘密配置及凭据引用，认证头由系统安全存储提供。文件协议只在会后上传，实时协议直接消费已落盘 PCM 连续副本；不经过等待本地 VAD 结束的预览入口。HTTP/WS 错误仅暴露静态错误类别；响应、令牌、音频、文本不得进入遥测。协议转换、重采样、分块只读事实 PCM，缺少服务时间戳时明确使用音频区间级时间。

本地预览复用单个 SenseVoice Engine 与现有采样缓存，进行中语音周期性生成临时字幕，片段完成后修订同一行。只保留最新待执行临时任务；过期返回不得覆盖稳定结果。应用限制单窗样本数，不把 VAD 最大语音参数当硬切上限。暂停先完成事实写入和有界预览交付，再收尾 VAD；结束仍优先封存录音。

安装前确认后沿用 Alpha 全清；数据库与数据代同时向前调整，清理包括本应用安全凭据。固定本地资源、三平台发布与原生加载门禁保持有效。
