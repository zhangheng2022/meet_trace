...
- 新统一版本序列从跨平台共享构建号 `2001` 开始并连续递增。Android 继续使用 `--split-per-abi`：传给 Flutter 的基础构建号为共享构建号减 `2000`，默认 ARM64 ABI 偏移后的实际 `versionCode` 与 iOS、Windows 构建号一致。既有 Android Alpha.5 的系统版本码 `2405` 不支持升级到新序列首包 `2001`；新序列内的自动更新仍属于 P0。
...
| AT-14 | 新序列 Alpha 安装升级 | `2001` 及后续版本升级时，提升数据代并全清旧数据后重走初始化；无残留旧数据库、音频、模型或检查点。既有 Android Alpha.5 `2405 → 2001` 不在兼容范围内 |
...
| AT-25 | 三平台自动更新 | 新统一序列只发现已公开批准的新版本；校验失败不安装；录音/最终处理期间不强制退出；数据代清理前明确警告；不允许降级。Android 不要求 Alpha.5 `2405 → 2001` 升级 |
