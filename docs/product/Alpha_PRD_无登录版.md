...
- 最终 ASR 为每个语音区间加入前后各 200 ms 上下文，超过 15 秒时使用 500 ms 重叠窗口；全静音会议直接生成空的已完成快照，不初始化 SenseVoice。
- 最终 VAD 创建、扫描或输出校验失败时，必须回退为从头处理完整事实 PCM 的固定 15 秒窗口，不能把 VAD 故障判成静音或丢失录音内容。
