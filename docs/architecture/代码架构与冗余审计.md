...
项目继续遵循 `View → ViewModel → Use Case / Port → Repository / Service`。Domain 未反向依赖 Data，UI 功能代码未直接调用 ONNX、数据库或 HTTP；事实录音与推理链仍保持隔离。`lib/` 当前共有 170 个 Dart 文件、23,632 行，结构性冗余本轮按六个阶段收敛。

本轮确认并处理的重点不是“文件数量”，而是所有权与事实源：删除失去产品入口的录音 Spike；把最终转录和说话人分离的时间段映射统一到一个 Domain 算法；把发布评估器移出运行时 Domain；把会议列表、详情 View 和详情 ViewModel 从共享私有作用域的 `part` 拆成显式 library；移除已退出产品范围的 AI 总结历史文档；停止跟踪 Graphify 缓存和日期备份。
...
| `data/` | 60 | 9,331 | 存储、下载、音频、ASR 与说话人实现 |
| `ui/` | 53 | 9,701 | Forui 页面、展示状态和显式功能组件 |
...
- 详情 ViewModel 的状态所有权仍集中在 `MeetingDetailViewModel`，转录、音频、结果动作三个分区通过命名明确的内部协作 API 工作，不再依赖跨文件可见的私有字段。
...
- Graphify 活动图谱不再把日期备份、缓存或已删除的总结方案当作并列知识来源。
...
- `flutter test`：462 项全部通过。
- Markdown 本地链接检查与 `git diff --check`：通过。
- OCR workspace review：25 个源码、测试与工具文件进入审查；Graphify 活动图、标签、报告、缓存和日期备份按生成物精确排除，并单独核验被删来源；无未解决的 Critical/High。
- `graphify update .`：活动图谱更新为 5,691 nodes / 7,684 edges / 374 communities；已删除的 Step 16 来源不再存在于活动 `graph.json`。CLI 提示 377 个保存标签与新 community 数不一致，未运行依赖外部 LLM 的重新命名；这不恢复已删除来源。
