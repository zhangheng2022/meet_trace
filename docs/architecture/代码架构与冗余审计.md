...
当前项目的主干分层正确，且由自动化测试约束：Domain 不反向依赖 Data，功能 UI 不直接依赖 Repository/Service 具体实现，录音写入与 ASR 预览保持解耦。现阶段主要问题不是“缺少架构”，而是迁移后的旧入口没有退出、资源初始化子系统形成循环依赖，以及组合根和少数业务/页面文件持续膨胀。
...
| 2. 循环依赖 | `SileroVadManifest` 下沉到 data model，data import 图无环 | 通用 import cycle 检测 |
...
分阶段修复完成后，`lib/` 共 156 个 Dart 文件：

| 层 | 文件数 | 总行数 | 观察 |
|---|---:|---:|---|
| `app/` | 7 | 737 | 组合根已按存储、运行时和会议能力拆分，顶层仅负责装配 |
| `domain/` | 42 | 3,643 | 端口与用例边界清楚，已移除重复模型选择用例 |
| `data/` | 52 | 7,194 | ASR、下载和存储实现最密集，共享安装事务已提取 |
| `ui/` | 49 | 7,579 | 已有共享组件与响应式骨架，少量高耦合 `part` 文件仍待后续迁移 |
| `theme/` | 6 | 648 | 颜色、排版、间距和尺寸令牌集中 |

当前依赖图没有跨层反向导入，`data` 本地 import 图也没有循环。修复前唯一的源码循环是：
...
SQLite 中 `requested_model_id` 与 `model_fallback_reason` 暂按历史审计兼容字段保留。本轮不以一次架构重构夹带破坏性 schema 迁移；若确认现有 Alpha 数据无需兼容，应单独设计迁移、恢复与回滚验证后删除。
...
已加入 Data→UI/App、Domain Flutter/Forui、旧 facade、data import cycle、具体 Engine 泄漏和顶层组合根扇出检查。
...
- `dart format lib test integration_test`：243 个文件，格式化完成。
- `flutter analyze`：0 issue。
- `flutter test`：328 项全部通过。
...
- OCR workspace review：79 个源码及测试文件进入审查；明确排除 `graphify-out/`、`build/` 生成物，Markdown 由人工逐项核对。修复 2 个恢复性问题后，无未解决的 Critical/High。
- `graphify update .`：代码图更新为 4,665 nodes / 6,275 edges / 375 communities。代码关系已同步；CLI 仍提示 Markdown 语义更新需要 AI 管线，且已有标签数量与当前 communities 不一致，不能把文档语义图视为完全同步。

## 8. 文档关系

- 产品范围：[PRD V0.7](../会迹_MeetTrace_Alpha_PRD_无登录版.md)
- 技术实现：[端侧 SenseVoice 转录技术方案](../端侧SenseVoice转录技术方案.md)
- 交付顺序：[Codex Alpha 开发步骤](../Codex_Alpha_开发步骤.md)
- 发布证据：[质量证据索引](../quality/README.md)
- 视觉规范：[DESIGN.md](../../DESIGN.md)

本审计是当前代码快照的维护建议。任何会改变 P0、模型集合、资源分发、录音连续性或验收门槛的动作，必须先更新 PRD。
