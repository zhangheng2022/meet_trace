...
项目继续遵循 `View → ViewModel → Use Case / Port → Repository / Service`。Domain 未反向依赖 Data，UI 功能代码未直接调用 ONNX、数据库或 HTTP；事实录音与推理链仍保持隔离。`lib/` 当前共有 168 个 Dart 文件、23,461 行，结构性冗余本轮按六个阶段收敛，并在复查中继续删除伪边界与失效实现。

本轮确认并处理的重点不是“文件数量”，而是所有权与事实源：删除失去产品入口的录音 Spike；把最终转录和说话人分离的时间段映射统一到一个 Domain 算法；把发布评估器移出运行时 Domain；把会议列表与详情 View 从共享私有作用域的 `part` 拆成显式 library；把循环回指的详情子 ViewModel 收敛为单一状态拥有者；删除无生产入口的说话人降级类与预览 FanOut；统一存储字节格式化；移除已退出产品范围的 AI 总结历史文档和查询记忆；停止跟踪 Graphify 缓存和日期备份。
...
| `domain/` | 43 | 3,104 | 业务模型、Port、纯 Dart 用例和共享映射算法 |
| `data/` | 60 | 9,297 | 存储、下载、音频、ASR 与说话人实现 |
| `ui/` | 51 | 9,564 | Forui 页面、展示状态和显式功能组件 |
...
- Graphify 活动图谱不再把日期备份、缓存、历史 Step 证据或已删除的总结方案当作并列知识来源；12 份涉及退出范围的旧查询记忆已从当前工作树移除，历史仍由 Git 保存。

### 阶段 6：审计与图谱刷新

- 本文替换 2026-08-05 的 V0.9 前快照，统计、产品版本和未完成风险均以 2026-08-07 工作树为准。
- 最终格式化、全量静态检查、测试、OCR 与 `graphify update .` 的结果在本轮验证结束后记录于下节。
...
- `dart format lib test tool`：277 个文件，无格式漂移。
- `flutter analyze`：0 issue。
- `flutter test`：463 项全部通过。
- Markdown 本地链接检查与 `git diff --check`：通过。
- OCR workspace review：13 个源码、测试与配置文件进入审查；Graphify 活动图、标签、报告与记忆按生成物精确排除，并单独核验被删来源和审计文档；无未解决的 Critical/High。
- `graphify update .`：活动图谱更新为 5,538 nodes / 7,514 edges / 361 communities；6 份历史 Step、4 个已删除详情 facade/state 文件及退役概念均不再存在于活动 `graph.json`。`graphify reflect` 从剩余 73 份记忆重建学习侧车，旧总结与降级实现不再是 preferred source。
...
本文是当前结构快照，不代表候选版本已通过 Android/iOS 发布验收；代码边界显著变化后必须重跑统计、验证、OCR 和 Graphify，并同轮更新。
