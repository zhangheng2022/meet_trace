...
分阶段修复完成后，五个分层目录共 157 个 Dart 文件；加上 `lib/main.dart`，`lib/` 共 158 个 Dart 文件：

| 层 | 文件数 | 总行数 | 观察 |
|---|---:|---:|---|
| `app/` | 7 | 737 | 组合根已按存储、运行时和会议能力拆分，顶层仅负责装配 |
| `domain/` | 42 | 3,643 | 端口与用例边界清楚，已移除重复模型选择用例 |
| `data/` | 53 | 7,198 | ASR、下载和存储实现最密集，共享安装事务与下载契约类型已提取 |
...
- `dart format lib test integration_test`：245 个文件，无格式漂移。
- `flutter analyze`：0 issue。
- `flutter test`：337 项全部通过；其中包括裸相对 import 守卫自验证，以及 8 项 `SherpaOnnxAsrEngine` 核心路径单元测试。
...
- OCR workspace review：本轮循环依赖修复与 Engine 测试共 13 个源码及测试文件进入审查；明确排除 `graphify-out/`、`build/` 生成物，Markdown 由人工核对，无未解决的 Critical/High。
