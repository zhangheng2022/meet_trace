...
2026-08-03 清理冲刺项 1 已完成遗留字段删除：产品决策采用 Alpha 升级全清数据策略（见 PRD V0.8 FR-004）。`LocalDataGenerationGate` 在打开数据库与初始化运行资源之前校验数据代标记，缺失或内容不一致时清空整个数据根目录并重走首次初始化；文件系统读取异常只阻断启动、不执行清理。schema 升至 v6 并删除 `requested_model_id`、`model_fallback_reason`，`Meeting` 模型同步移除请求模型与回退原因字段；架构守卫（`legacy_schema_guard_test`，带自验证夹具）禁止遗留列名重新进入 `lib/`。数据库层对 v1～v5 升级保留拒绝作为兜底。
