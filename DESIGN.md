...
- 冷启动以“正在准备会迹”承接本地数据恢复，完成后进入会议首页；首页与在线配置不等待本地模型资源。
- 选择本地能力时进入独立的资源准备流程；Logo、标题与内容位置保持稳定，仅更新阶段、状态、下载进度与必要操作。
- 数据读取失败在启动页提供恢复操作；空间不足、网络确认和模型资源失败只阻断所选本地能力，不能阻断历史会议或在线来源。
...
首页录音条件条必须读取麦克风权限、可用存储与默认模型的真实预检结果，区分检查中、已就绪、需要处理和检查失败。预检条使用稳定的两层排版：第一行是当前状态标题，第二行是事实说明；不得把标题、存储事实和模型信息串成同一段长句。就绪状态的第二行同时说明事实音频保存在本机和当前默认转录来源；在线来源的音频发送需另行会前确认。开始会议时再次预检，首页结果不得替代录音服务的最终权限与空间检查。

### Recorder Instrument

录音页顶部栏显示当前会议标题，录音仪器按固定顺序展示：录音状态、唯一的大号计时器、真实麦克风输入波形，以及无内嵌卡片的两行事实摘要（事实音频状态、锁定模型）。手机端使用 12px 仪器内边距和 48px 计时器，平板端保留 24px 内边距和 56px 计时器。暂停/恢复与“结束会议”固定在底部操作栏；“实时转录仅供参考”只在实时转录区出现一次。首页的“开始会议”先明确选择本地或用户配置的在线来源；标记全局默认，允许本场选择，在线须确认接收服务、音频发送与最终稿额外识别费用。本地资源按需准备，不阻断首页。创建会议时按本地开始时间生成稳定标题，例如 `2026-08-03 14:30 会议`；开始后冻结来源和参数，失败继续录音，会后可明确换来源重转录。波形只能由已经写入事实音频的 PCM 音量驱动，以底部基线和仅向上生长的单边波形确认麦克风输入；手机高 52px，平板高 64px。新样本从当前画面状态开始 140ms 可中断过渡，并以短窗自适应增益增强安静输入的可见差异；系统要求减弱动态时立即更新。不得显示“时/分/秒”、重复时间刻度或把随机动画伪装为真实音频。
...
底部操作栏贴合安全区，以记录白页和顶部细分隔线与内容区分离。首页只保留占满可用宽度的事实墨色“开始会议”主操作；录音页只保留“暂停/继续”和“结束会议”，宽屏也不重复事实音频说明。“事实音频保存在本机”并入顶部真实预检，在线音频发送由选源流程另行确认；“实时转录仅供参考”只在录音语境中出现一次。
