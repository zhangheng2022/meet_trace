...
- Android：正式候选构建签名的 armeabi-v7a、arm64-v8a、x86_64 split 与 universal APK；ARM 包在对应 Firebase 真机以 `--no-resign` 验证，x86_64 在 CI 模拟器验证。
...
- 三平台使用同一营销版本和共享构建号。
- GitHub Release 只含四个 Android APK 与公开候选清单；IPA、MSIX 和详细证据留在平台或短期 Artifact。
