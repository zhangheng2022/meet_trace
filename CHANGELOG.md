# 更新日志

本文件记录会迹面向用户的版本变化，并从 `v1.0.0-alpha.13` 开始维护。更早版本请查看 [GitHub Releases](https://github.com/zhangheng2022/meet_trace/releases)。

每项使用“新增、变更、修复、安全、兼容性、已知问题”之一作为前缀；内部重构、依赖更新和 CI 调整仅在影响用户或发布可靠性时记录。

## [Unreleased]

- 新增：每场选择本地或自定义在线转录，支持 Audio Transcriptions、Chat Audio 与 Realtime Transcription 协议，不限制在线模型 ID；认证信息保存在系统安全存储。
- 新增：在线录音结束后从完整事实音频生成最终稿；会后可手动换来源重转录，失败保留旧稿和录音。实时字幕与最终稿可能分别计费。
- 变更：本地权重改为按需准备，未下载也可访问首页、历史和在线配置。
- 修复：本地长句可显示并修订临时字幕，预览拥堵时优先最新窗口；独立限制连续语音窗口，迟到结果不覆盖新字幕，暂停时交付已落盘音频尾部。
- 兼容性：本次提高数据代，沿用 Alpha 全清策略；安装前须确认清除历史会议、录音、模型、设置及本应用在线凭据，不执行数据迁移。

## [1.0.0-alpha.14] - 2026-09-04

- 新增：Android 发布同时提供 armeabi-v7a、arm64、x86_64 split 与 universal APK；README 默认推荐 arm64，应用内更新按设备 ABI 下载对应 split。

## [1.0.0-alpha.13] - 2026-09-03

- 变更：设置页改为贴近原生设置列表的单列布局，合并外观与语言区域，并将麦克风、存储与诊断等低频内容收进“更多设置”。
- 变更：移除首页一次性 Sentry 告知，保留设置页远程诊断披露与退出开关。
- 新增：项目提供可审查的更新日志，并由发布流程同步到 GitHub Release 与 TestFlight。
