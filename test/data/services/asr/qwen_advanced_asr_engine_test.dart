...
void main() {
  ...
  setUp(() async {
    ...
    _installActiveQwen(installations, tempDirectory.path);
  });
  ...
  test('只从活动且已校验的高级版本创建配置并获取租约', () async {
    ...
    expect(
      engine.descriptor,
      same(AsrModelRegistry.alpha.requireById(qwenAdvancedModelId)),
    );
    final config = workers.configs.single;
    expect(config.kind, SherpaOnnxRecognizerKind.qwen3Asr);
    expect(
      config.convFrontendPath,
      p.join(tempDirectory.path, 'conv_frontend.onnx'),
    );
    expect(config.encoderPath, p.join(tempDirectory.path, 'encoder.int8.onnx'));
    expect(config.decoderPath, p.join(tempDirectory.path, 'decoder.int8.onnx'));
    expect(config.tokenizerPath, p.join(tempDirectory.path, 'tokenizer'));
  ...
  test('未安装、校验失败或非活动版本不能创建 Engine', () async {
    ...
    await expectLater(
      ...
      throwsA(_engineFailure('asr.qwen.model_not_active')),
    );
    ...
    await expectLater(
      ...
      throwsA(_engineFailure('asr.qwen.model_not_verified')),
    );
  ...
  test('同一 owner 的活动租约冲突时拒绝重复创建', () async {
    ...
    await expectLater(
      ...
      throwsA(_engineFailure('asr.qwen.lease_conflict')),
    );
    expect(workers.createCalls, 0);
  });
  ...
  test('不支持设备、临界内存和临界温控分别阻止初始化', () async {
    final cases = <(AsrDeviceRiskState, String)>[
      ...
      (
        ...
        'asr.qwen.memory_pressure_critical',
      ),
      (
        const AsrDeviceRiskState(
          support: AsrDeviceSupport.supported,
  ...
  test('事件与最终快照协议可通过统一 AsrEngine 使用', () async {
    ...
    expect(preview.modelId, qwenAdvancedModelId);
    expect(snapshot.actualModelId, qwenAdvancedModelId);
  ...
  test('超过 15 秒的输入不会进入 Qwen adapter', () async {
    final engine = await _createEngine(
      installations: installations,
      leases: leases,
      risks: risks,
    ...
    await expectLater(
      ...
      throwsA(_engineFailure('asr.qwen.window_too_long')),
    );
  ...
  test('推理失败保持高级模型身份且不触发自动模型切换', () async {
    ...
    await expectLater(
      engine.acceptAudio(Float32List(16000), sampleRate: 16000, startMs: 0),
      throwsA(_engineFailure('asr.official.inference_failed')),
    );

    expect(engine.descriptor.modelId, qwenAdvancedModelId);
    expect(workers.configs.single.kind, SherpaOnnxRecognizerKind.qwen3Asr);
...
Future<QwenAdvancedAsrEngine> _createEngine({
  ...
  return QwenAdvancedAsrEngine.create(
    installations: installations,
    leases: leases,
    riskMonitor: risks,
    ownerId: ownerId,
