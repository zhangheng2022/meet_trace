...
- Windows Microsoft Store 候选固定使用 `SENTRY_ENABLED=false`，不启动 Sentry 远程诊断。候选只包含应用运行文件和公开元数据，不包含会议内容；上传 Partner Center 后由 Microsoft 执行认证、签名和分发。SignPath 申请材料仍保留，但当前不向 SignPath 提交 Windows 候选。
- iOS TestFlight 和 Microsoft Store 分发分别受 Apple 与 Microsoft 的账户、商店和诊断设置约束。未来自动更新正式接线后，只能访问公开批准的签名更新清单和固定 Store 产品入口；在接线与隐私选项验收完成前不得扩大公开分发。

## 诊断控制

公开 Windows Store 候选不启用 Sentry。Android/iOS Alpha 的 Sentry 开关是构建期配置，目前没有应用内遥测开关；因此这些平台的安装说明和发布说明必须明确披露远程诊断。需要完全关闭 Sentry 的测试构建必须使用 `SENTRY_ENABLED=false`，且不得伪装成同一正式候选。
