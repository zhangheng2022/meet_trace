...
应用私有目录保存事实 PCM、会议元数据、转录、说话人标签、处理状态、检查点、设置和模型。MeetTrace 不提供同步，也不将会议内容发送到 MeetTrace 业务服务器。用户明确选择在线来源后，派生音频和必要识别参数直接发送至用户配置的服务，实时字幕与会后完整音频识别可能分别计费。API Key 与认证头仅保存在本机系统安全存储。

用户主动分享文本，或二次确认音频分享后，选定内容会进入系统分享面板；之后由接收应用及其隐私政策负责。

## 网络访问

在线转录每场先展示服务与上传范围；自定义服务的处理和保留规则由该服务负责。配置测试经用户操作发送内置测试音频，不使用历史会议或麦克风内容。文件接口仅在会后发送音频。
...
- 删除本地会议不等于删除在线服务已经接收的副本；服务端保留与删除以该服务政策为准。
- 删除会议会删除该会议的音频、转录、说话人标签、处理记录和应用自有临时分享文件。
- 破坏性 Alpha 可在安装前确认后清除全部应用数据和本应用安全存储凭据并重新初始化；卸载也会删除应用私有目录。已分享副本不受 MeetTrace 控制。
