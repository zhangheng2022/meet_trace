...
## 更新摘要
**所做更改**
- 更新了 Meeting 模型章节，移除了 requestedModelId 和 modelFallbackReason 字段的描述
- 简化了模型选择逻辑说明，强调单模型基线设计
- 更新了数据库架构说明，移除遗留列引用
- 更新了数据序列化格式，移除相关字段
- 更新了使用示例，移除多模型选择和回退机制相关内容
- 添加了架构守卫测试说明，确保遗留代码不会重新引入
...
**重要更新**：Meeting 模型已简化为单模型基线设计，移除了多模型选择和回退机制，不再支持 requestedModelId 和 modelFallbackReason 字段。
...
## 核心组件
- Meeting：会议实体，封装生命周期状态、录音模型选择、音频元数据、激活的转录快照与摘要 ID、错误码等；提供不可变更新方法与状态机转换。**已简化为单模型设计，移除了多模型选择和回退机制**。
...
- 属性
  ...
  - **单模型选择**：recordingModelId、recordingModelVersion、recordingModelLanguage、recordingModelUseInverseTextNormalization
...
- 业务方法
  ...
  - changeRecordingModel(...)：在未锁定状态下更换录音模型与参数，**不再需要回退原因参数**
...
**更新**：Meeting 模型已简化为单模型基线设计，移除了 requestedModelId 和 modelFallbackReason 字段，不再支持多模型选择和回退机制。
...
- JSON 字段建议（基于领域模型与数据库列名推导）
  - Meeting
    - id, title, createdAt(ms), startedAt(ms), endedAt(ms), status, audioPath, audioDurationMs, recordingModelId, recordingModelVersion, recordingModelLanguage, recordingModelUseInverseTextNormalization, activeTranscriptSnapshotId, activeSummaryId, lastErrorCode
  - TranscriptSnapshot
...
**更新**：移除了 requested_model_id 和 model_fallback_reason 字段，符合单模型基线设计。

章节来源
- [lib/data/models/storage/storage_mappers.dart:9-29](file://lib/data/models/storage/storage_mappers.dart#L9-L29)
- [lib/data/repositories/sqflite_transcript_repository.dart:191-227](file://lib/data/repositories/sqflite_transcript_repository.dart#L191-L227)
- [lib/data/services/storage/app_database.dart:59-79](file://lib/data/services/storage/app_database.dart#L59-L79)

### 数据验证规则与约束条件
- Meeting
  - 时间顺序与长度约束；**单模型约束**；录音模型锁定；必填字段非空
- TranscriptSegment
  - 时间区间合法；置信度范围；必填字段非空
- TranscriptSnapshot
...
### 使用示例（JSON 与 Dart）
- JSON 示例（Meeting）
  - 字段：id、title、createdAt(ms)、startedAt(ms)、endedAt(ms)、status、audioPath、audioDurationMs、recordingModelId、recordingModelVersion、recordingModelLanguage、recordingModelUseInverseTextNormalization、activeTranscriptSnapshotId、activeSummaryId、lastErrorCode
...
- Dart 示例（Meeting）
  - 创建：Meeting(id, title, createdAt, status, audioDurationMs, recordingModelId, recordingModelVersion, ...)
  ...
  - 更换录音模型：meeting.changeRecordingModel(recordingModelId, recordingModelVersion, recordingModelLanguage, recordingModelUseInverseTextNormalization)

**更新**：changeRecordingModel 方法不再需要 fallbackReason 参数。
...
- **架构守卫**：legacy_schema_guard_test.dart 确保遗留列名不会重新引入到 lib/ 目录中

**更新**：新增了架构守卫测试，防止遗留的多模型选择代码重新引入。
...
## 结论
本文件系统化梳理了会迹项目的核心数据模型与状态机，明确了各实体的属性、约束、序列化格式与数据库映射，提供了面向开发与运维的参考与实践建议。通过不可变对象与严格状态机，确保数据一致性与可维护性。**最新的单模型基线设计进一步简化了系统复杂度，移除了多模型选择和回退机制，提升了系统的稳定性和可维护性**。
...
- 术语
  ...
  - 状态机：限定状态间合法转换的规则集合
  - **单模型基线**：Meeting 模型只保留单一录音模型，不再支持多模型选择和回退机制
