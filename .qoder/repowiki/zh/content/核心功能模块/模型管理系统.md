...
subgraph "数据层"
F["DownloadableModelService"]
G["ModelFileVerifier"]
H["RuntimeArtifactInstallTransaction"]
I["ModelManifestParser"]
J["SqfliteModelUsageLeaseRepository"]
end
A --> B
C --> I
D --> E
...
- [lib/data/services/models/downloadable_model_service.dart:1-722](file://lib/data/services/models/downloadable_model_service.dart#L1-L722)
...
Caller->>Service : download(descriptor, manifest, onProgress)
...
Service-->>Caller : 返回安装路径/是否已安装/是否续传
...
- 进度与取消
  - 通过回调报告 phase、completedBytes、totalBytes；支持 ModelDownloadCancellationToken 中断并转为暂停状态。
- 删除保护
...
## 结论
该模型管理系统以清晰的领域模型与状态机为核心，结合严格的清单解析与完整性校验，实现了可靠的下载、校验、原子切换与删除保护。通过快速就绪检测与断点续传提升用户体验，借助租约机制保障并发安全。未来可扩展多模型注册表与选择策略，完善监控与日志体系，支撑更复杂的模型生态。
