# 扩展开发指南

<cite>
**本文引用的文件**   
- [lib/domain/ports/asr_engine.dart](file://lib/domain/ports/asr_engine.dart)
- [lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart](file://lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart)
- [lib/app/meettrace_storage_dependencies.dart](file://lib/app/meettrace_storage_dependencies.dart)
- [lib/data/services/storage/local_data_control_service.dart](file://lib/data/services/storage/local_data_control_service.dart)
- [lib/data/services/models/http_model_file_downloader.dart](file://lib/data/services/models/http_model_file_downloader.dart)
- [lib/theme/theme.dart](file://lib/theme/theme.dart)
- [lib/theme/colors.dart](file://lib/theme/colors.dart)
- [lib/theme/style.dart](file://lib/theme/style.dart)
- [lib/theme/icons.dart](file://lib/theme/icons.dart)
- [.agents/skills/grill-with-docs/SKILL.md](file://.agents/skills/grill-with-docs/SKILL.md)
- [.agents/skills/grill-with-docs/agents/openai.yaml](file://.agents/skills/grill-with-docs/agents/openai.yaml)
- [.agents/skills/grilling/SKILL.md](file://.agents/skills/grilling/SKILL.md)
- [.agents/skills/grill-me/SKILL.md](file://.agents/skills/grill-me/SKILL.md)
- [skills-lock.json](file://skills-lock.json)
</cite>

## 更新摘要
**所做更改**   
- 新增AI技能系统集成章节，详细说明grill-with-docs技能的配置和使用方法
- 更新技能架构模式说明，包括显式命令调用和安全策略
- 扩展示例部分，增加技能开发的完整指导
- 更新依赖关系分析，包含技能系统组件

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [AI技能系统集成](#ai技能系统集成)
7. [依赖关系分析](#依赖关系分析)
8. [性能考量](#性能考量)
9. [故障排查指南](#故障排查指南)
10. [结论](#结论)
11. [附录：扩展示例与最佳实践](#附录扩展示例与最佳实践)

## 简介
本指南面向希望为"会迹"项目扩展能力的开发者，覆盖以下主题：
- 新增 ASR 引擎：实现 AsrEngine 接口、模型格式支持、性能优化策略
- 自定义存储后端：StorageProvider 接口（以现有 Repository/Service 抽象为参考）、数据迁移、备份恢复
- 第三方服务集成：HTTP 客户端配置、认证机制、错误处理
- UI 组件扩展：Forui 主题定制、平台特定实现
- **AI技能系统集成**：技能架构模式、显式命令调用、安全策略配置
- 插件开发模式：依赖注入、生命周期管理、配置管理
- 完整扩展示例与最佳实践建议

## 项目结构
本项目采用分层架构与清晰的职责划分：
- domain：领域模型与端口（如 AsrEngine、AsrDeviceRiskMonitor）
- data：数据层（仓库、服务、适配器、下载器、存储布局等）
- app：应用装配与依赖注入（StorageDependencies 等）
- theme：UI 主题与样式（基于 Forui）
- **.agents/skills**：AI技能系统，包含各种专业技能和配置

```mermaid
graph TB
subgraph "领域层(domain)"
A["AsrEngine 接口<br/>AsrDeviceRiskMonitor"]
B["Transcript / Model 描述符"]
end
subgraph "数据层(data)"
C["SherpaOnnxAsrEngine 实现"]
D["HttpModelFileDownloader"]
E["LocalDataControlService"]
F["AppFileLayout / AppDatabase"]
end
subgraph "应用层(app)"
G["StorageDependencies(装配)"]
end
subgraph "主题(theme)"
H["FThemeData(light/dark)<br/>colors/style/icons"]
end
subgraph "AI技能系统(.agents/skills)"
I["grill-with-docs 技能<br/>grilling 技能<br/>grill-me 技能"]
J["技能配置(openai.yaml)"]
K["skills-lock.json"]
end
A --> C
C --> D
C --> F
G --> F
G --> E
H --> |被 UI 使用| G
I --> J
J --> K
```

**图表来源**
- [lib/domain/ports/asr_engine.dart:133-174](file://lib/domain/ports/asr_engine.dart#L133-L174)
- [lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart:19-41](file://lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart#L19-L41)
- [.agents/skills/grill-with-docs/SKILL.md:1-8](file://.agents/skills/grill-with-docs/SKILL.md#L1-L8)
- [.agents/skills/grill-with-docs/agents/openai.yaml:1-6](file://.agents/skills/grill-with-docs/agents/openai.yaml#L1-L6)
- [skills-lock.json:166-171](file://skills-lock.json#L166-L171)

**章节来源**
- [lib/domain/ports/asr_engine.dart:133-174](file://lib/domain/ports/asr_engine.dart#L133-L174)
- [lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart:19-41](file://lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart#L19-L41)
- [.agents/skills/grill-with-docs/SKILL.md:1-8](file://.agents/skills/grill-with-docs/SKILL.md#L1-L8)

## 核心组件
- AsrEngine 与 AsrEngineFactory：定义 ASR 引擎的统一能力与工厂创建方式，包含设备风险监测、事件流、指标统计、音频窗口识别与会话终态生成。
- SherpaOnnxAsrEngine：具体引擎实现，封装窗口切分、推理、进度上报、诊断与异常映射。
- StorageDependencies：集中装配数据库、文件布局与各仓库实例，提供统一生命周期管理。
- LocalDataControlService：本地存储用量测量与基础控制。
- HttpModelFileDownloader：模型文件下载器，支持断点续传、HTTPS 校验、取消令牌与进度回调。
- Theme（Forui）：light/dark 主题、颜色、图标与样式组合。
- **AI技能系统**：基于SKILL.md和openai.yaml配置的专业技能集合，支持显式命令调用和安全策略。

**章节来源**
- [lib/domain/ports/asr_engine.dart:133-174](file://lib/domain/ports/asr_engine.dart#L133-L174)
- [lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart:19-41](file://lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart#L19-L41)
- [.agents/skills/grill-with-docs/SKILL.md:1-8](file://.agents/skills/grill-with-docs/SKILL.md#L1-L8)

## 架构总览
下图展示了 ASR 引擎在系统中的位置与关键交互：上层通过 AsrEngineFactory 创建引擎，引擎内部通过适配器与底层推理库交互，同时暴露事件流与指标；存储与下载服务由应用层装配并注入到相关组件中。

```mermaid
sequenceDiagram
participant Caller as "调用方"
participant Factory as "AsrEngineFactory"
participant Engine as "AsrEngine(实现)"
participant Adapter as "底层适配器"
participant Store as "存储服务"
participant Downloader as "下载器"
Caller->>Factory : create(modelId, modelVersion, ...)
Factory-->>Caller : AsrEngine 实例
Caller->>Engine : initialize()
Engine->>Adapter : 初始化
Engine-->>Caller : 就绪
Caller->>Engine : acceptAudio(samples, sampleRate, startMs)
Engine->>Adapter : recognize(...)
Adapter-->>Engine : 文本片段
Engine-->>Caller : events 流(TranscriptEvent)
Caller->>Engine : finalizeMeeting(source, meetingId)
Engine->>Store : 读取/校验 PCM
Engine->>Downloader : 按需拉取模型(若需要)
Engine-->>Caller : finalizationProgress + TranscriptSnapshot
```

**图表来源**
- [lib/domain/ports/asr_engine.dart:133-174](file://lib/domain/ports/asr_engine.dart#L133-L174)
- [lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart:109-158](file://lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart#L109-L158)
- [lib/data/services/models/http_model_file_downloader.dart:17-76](file://lib/data/services/models/http_model_file_downloader.dart#L17-L76)

## 详细组件分析

### 新增 ASR 引擎（AsrEngine 接口实现）
- 目标：在不改动上层业务的前提下，接入新的语音识别引擎（例如其他 ONNX Runtime、端侧推理框架）。
- 关键点：
  - 实现 AsrEngine 接口：initialize、acceptAudio、finalizeMeeting、cancel、dispose、事件与指标流。
  - 遵循窗口大小与采样率约束，保证可预测的内存与延迟。
  - 正确映射底层异常为 AsrEngineException/AppFailure，便于统一错误处理。
  - 暴露 AsrDeviceRiskState 变化，供上层做降级或提示。
  - 维护 metrics 与 diagnostics，便于性能分析与问题定位。

```mermaid
classDiagram
class AsrEngine {
+descriptor
+initialize()
+events
+finalizationProgress
+deviceRisk
+deviceRisks
+metrics
+diagnostics
+acceptAudio(samples, sampleRate, startMs)
+finalizeMeeting(source, meetingId, snapshotId)
+cancel()
+dispose()
}
class AsrEngineFactory {
+create(modelId, modelVersion, language, useInverseTextNormalization)
}
class SherpaOnnxAsrEngine {
+descriptor
+initialize()
+acceptAudio(...)
+finalizeMeeting(...)
+cancel()
+dispose()
-_adapter
-_riskMonitor
-_beforeOperation
-_onDispose
}
AsrEngine <|.. SherpaOnnxAsrEngine
AsrEngineFactory ..> AsrEngine : "创建"
```

**图表来源**
- [lib/domain/ports/asr_engine.dart:133-174](file://lib/domain/ports/asr_engine.dart#L133-L174)
- [lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart:19-41](file://lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart#L19-L41)

**章节来源**
- [lib/domain/ports/asr_engine.dart:133-174](file://lib/domain/ports/asr_engine.dart#L133-L174)
- [lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart:109-158](file://lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart#L109-L158)
- [lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart:160-171](file://lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart#L160-L171)
- [lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart:174-294](file://lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart#L174-L294)

#### 模型格式支持与解码流程
- 输入要求：PCM16、单声道、固定采样率（示例实现使用 16kHz），窗口长度限制（示例实现最大 15s）。
- 解码路径：二进制 PCM 转 Float32 归一化样本，再送入推理引擎。
- 输出：按窗口返回文本片段，最终聚合为 TranscriptSnapshot。

```mermaid
flowchart TD
Start(["开始"]) --> Validate["校验采样率/通道/窗口长度"]
Validate --> Valid{"是否合法?"}
Valid --> |否| Error["抛出参数异常(AppFailure)"]
Valid --> |是| Decode["PCM16 -> Float32 归一化"]
Decode --> Recognize["调用推理引擎识别"]
Recognize --> Empty{"是否有文本?"}
Empty --> |否| RecordEmpty["记录空窗口指标"]
Empty --> |是| Emit["发出 TranscriptSegmentEvent"]
RecordEmpty --> Next["继续下一窗口"]
Emit --> Next
Next --> Finalize{"是否结束会话?"}
Finalize --> |否| Next
Finalize --> |是| BuildSnapshot["构建 TranscriptSnapshot"]
BuildSnapshot --> End(["结束"])
Error --> End
```

**图表来源**
- [lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart:533-576](file://lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart#L533-L576)
- [lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart:647-654](file://lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart#L647-L654)
- [lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart:174-294](file://lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart#L174-L294)

**章节来源**
- [lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart:533-576](file://lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart#L533-L576)
- [lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart:647-654](file://lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart#L647-L654)
- [lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart:174-294](file://lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart#L174-L294)

#### 性能优化建议
- 窗口切分：保持固定窗口大小，避免频繁 GC 与上下文切换。
- 资源复用：复用解码缓冲与推理上下文，减少分配开销。
- 并发控制：串行化窗口推理，避免竞争条件与状态不一致。
- 指标监控：利用 metrics.realTimeFactor 评估实时性，结合 diagnostics 定位瓶颈。
- 设备风险：根据 AsrDeviceRiskState 动态降级（如降低采样率、缩短窗口、关闭非关键功能）。

**章节来源**
- [lib/domain/ports/asr_engine.dart:81-110](file://lib/domain/ports/asr_engine.dart#L81-L110)
- [lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart:94-106](file://lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart#L94-L106)
- [lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart:468-499](file://lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart#L468-L499)

### 自定义存储后端（StorageProvider 思路）
说明：当前代码未直接出现名为 StorageProvider 的接口，但通过 Repository 与 Service 抽象实现了可扩展的数据访问。可按如下方式扩展：
- 定义抽象接口（参考现有 Repository/Port 风格）：
  - 读写会议、转录、模型安装、偏好设置、任务与摘要等
  - 提供 watchAll 流式变更通知
- 实现多后端：
  - Sqflite（已有）
  - SQLite/WAL 优化、索引设计
  - 云存储（对象存储+元数据表）
- 数据迁移：
  - 版本化 schema，启动时执行迁移脚本
  - 失败回滚与幂等性保障
- 备份恢复：
  - 增量快照与一致性检查
  - 跨平台路径与权限处理

```mermaid
classDiagram
class MeetingRepository {
+getById(id)
+save(meeting)
+delete(id)
+watchAll()
}
class TranscriptRepository {
+...
}
class ModelInstallationRepository {
+...
}
class LocalDataControlPort {
+measure()
}
MeetingRepository <|-- SqfliteMeetingRepository
TranscriptRepository <|-- SqfliteTranscriptRepository
ModelInstallationRepository <|-- SqfliteModelInstallationRepository
LocalDataControlPort <|-- LocalDataControlService
```

**图表来源**
- [lib/app/meettrace_storage_dependencies.dart:15-41](file://lib/app/meettrace_storage_dependencies.dart#L15-41)
- [lib/data/services/storage/local_data_control_service.dart:9-22](file://lib/data/services/storage/local_data_control_service.dart#L9-L22)

**章节来源**
- [lib/app/meettrace_storage_dependencies.dart:15-41](file://lib/app/meettrace_storage_dependencies.dart#L15-41)
- [lib/data/services/storage/local_data_control_service.dart:9-22](file://lib/data/services/storage/local_data_control_service.dart#L9-L22)

### 第三方服务集成（HTTP 客户端、认证、错误处理）
- HTTP 客户端配置：
  - 连接超时、空闲超时、User-Agent、Range 头支持
  - 强制 HTTPS（可通过配置开关）
- 认证机制：
  - 请求前注入 Token（Header/Cookie/签名）
  - 401 自动刷新或引导重新登录
- 错误处理：
  - 统一异常类型（如 DownloadableModelException）
  - 区分网络错误、服务端错误、范围不兼容等场景
  - 取消令牌支持中断长耗时操作

```mermaid
sequenceDiagram
participant Client as "调用方"
participant Downloader as "HttpModelFileDownloader"
participant Net as "HttpClient"
participant Server as "远端服务器"
Client->>Downloader : download(source, destinationPath, resumeFrom, expectedBytes, cancellation, onProgress)
Downloader->>Net : getUrl(source) + Headers(User-Agent, Range?)
Net-->>Server : HTTP 请求
Server-->>Net : 200 OK 或 206 Partial Content
Net-->>Downloader : 响应流
Downloader->>Downloader : 写入文件/更新进度
Downloader-->>Client : 结果(resumed, finalBytes)
Note over Downloader,Server : 失败时抛出带 code/message 的异常
```

**图表来源**
- [lib/data/services/models/http_model_file_downloader.dart:17-76](file://lib/data/services/models/http_model_file_downloader.dart#L17-L76)

**章节来源**
- [lib/data/services/models/http_model_file_downloader.dart:17-76](file://lib/data/services/models/http_model_file_downloader.dart#L17-L76)

### UI 组件扩展（Forui 主题与平台特定实现）
- 主题体系：
  - lightTheme/darkTheme 通过 FThemeData 组合 colors、typography、icons、style
  - 颜色 tokens 与系统 overlay 样式分离，便于深色模式适配
- 样式定制：
  - 圆角、边框宽度、页面内边距、聚焦轮廓等
  - 扩展 AppStyle/AppColors 以承载业务语义色
- 平台特定实现：
  - 触摸/桌面变体（touch 标志位）
  - 系统 UI 覆盖栏样式（状态栏、导航栏）

```mermaid
classDiagram
class FThemeData {
+colors
+typography
+icons
+style
+touch
}
class FColors {
+brightness
+systemOverlayStyle
+primary/background/foreground/...
+extensions
}
class FStyle {
+formFieldStyle
+focusedOutlineStyle
+sizes
+iconStyle
+tappableStyle
+borderRadius
+pagePadding
+shadow
+extensions
}
FThemeData --> FColors
FThemeData --> FStyle
```

**图表来源**
- [lib/theme/theme.dart:26-45](file://lib/theme/theme.dart#L26-L45)
- [lib/theme/colors.dart:6-25](file://lib/theme/colors.dart#L6-L25)
- [lib/theme/style.dart:8-45](file://lib/theme/style.dart#L8-L45)

**章节来源**
- [lib/theme/theme.dart:26-45](file://lib/theme/theme.dart#L26-L45)
- [lib/theme/colors.dart:6-25](file://lib/theme/colors.dart#L6-L25)
- [lib/theme/style.dart:8-45](file://lib/theme/style.dart#L8-L45)
- [lib/theme/icons.dart:6-29](file://lib/theme/icons.dart#L6-L29)

### 插件开发模式（依赖注入、生命周期、配置）
- 依赖注入：
  - 通过 StorageDependencies 集中装配数据库、仓库与服务
  - 工厂方法 create 负责初始化顺序与错误清理
- 生命周期：
  - 统一的 dispose 流程，确保资源释放与错误传播
  - 启动恢复服务（StartupRecoveryService）保障崩溃后一致性
- 配置管理：
  - 引擎参数（语言、反规范化、窗口大小、采样率）
  - 下载器参数（HTTPS 强制、超时、重试）
  - 主题参数（touch、颜色、图标）

```mermaid
flowchart TD
Init["应用启动"] --> CreateLayout["创建文件布局"]
CreateLayout --> OpenDB["打开数据库"]
OpenDB --> Recover["启动恢复服务"]
Recover --> BuildRepo["构建各仓库实例"]
BuildRepo --> ReturnDeps["返回 StorageDependencies"]
ReturnDeps --> Use["业务模块消费依赖"]
Use --> Dispose["应用退出/销毁"]
Dispose --> CloseDB["关闭数据库/释放资源"]
```

**图表来源**
- [lib/app/meettrace_storage_dependencies.dart:40-87](file://lib/app/meettrace_storage_dependencies.dart#L40-L87)
- [lib/app/meettrace_storage_dependencies.dart:89-96](file://lib/app/meettrace_storage_dependencies.dart#L89-L96)

**章节来源**
- [lib/app/meettrace_storage_dependencies.dart:40-87](file://lib/app/meettrace_storage_dependencies.dart#L40-L87)
- [lib/app/meettrace_storage_dependencies.dart:89-96](file://lib/app/meettrace_storage_dependencies.dart#L89-L96)

## AI技能系统集成

### 技能架构概述
项目的AI技能系统基于标准化的SKILL.md文件和openai.yaml配置文件，提供了强大的可扩展能力。每个技能都遵循统一的架构模式，支持显式命令调用以提高安全性。

```mermaid
graph TB
subgraph "技能目录结构"
A[".agents/skills/"]
B["grill-with-docs/"]
C["grilling/"]
D["grill-me/"]
E["impeccable/"]
end
subgraph "技能文件"
F["SKILL.md<br/>技能定义"]
G["agents/openai.yaml<br/>API配置"]
H["scripts/<br/>工具脚本"]
I["reference/<br/>参考文档"]
end
subgraph "配置管理"
J["skills-lock.json<br/>技能锁定"]
K["policy:<br/>allow_implicit_invocation: false"]
L["disable-model-invocation: true"]
end
A --> B
A --> C
A --> D
A --> E
B --> F
B --> G
C --> F
C --> G
D --> F
D --> G
E --> F
E --> H
E --> I
F --> J
G --> K
G --> L
```

**图表来源**
- [.agents/skills/grill-with-docs/SKILL.md:1-8](file://.agents/skills/grill-with-docs/SKILL.md#L1-L8)
- [.agents/skills/grill-with-docs/agents/openai.yaml:1-6](file://.agents/skills/grill-with-docs/agents/openai.yaml#L1-L6)
- [.agents/skills/grilling/SKILL.md:1-13](file://.agents/skills/grilling/SKILL.md#L1-L13)
- [skills-lock.json:166-171](file://skills-lock.json#L166-L171)

### grill-with-docs 技能详解

grill-with-docs 是一个专门用于设计和计划审查的技能，它结合了严格的面试流程和文档生成功能。该技能遵循项目的技能架构模式，具有以下特点：

#### 技能配置
```yaml
---
name: grill-with-docs
description: A relentless interview to sharpen a plan or design, which also creates docs (ADR's and glossary) as we go.
disable-model-invocation: true
---

Run a `/grilling` session, using the `/domain-modeling` skill.
```

#### API配置
```yaml
interface:
  display_name: "Grill with Docs"
  short_description: "Grill a design and write its docs"
policy:
  allow_implicit_invocation: false
```

#### 核心特性
- **显式命令调用**：通过 `disable-model-invocation: true` 禁用隐式模型调用，提高安全性
- **政策控制**：`allow_implicit_invocation: false` 确保只能通过显式命令调用
- **文档生成**：自动生成ADR（架构决策记录）和术语表
- **领域建模**：与 `/domain-modeling` 技能集成，提供专业的领域分析

### 技能注册与管理

项目使用 `skills-lock.json` 文件来管理和锁定所有已安装的技能：

```json
{
  "version": 1,
  "skills": {
    "grill-with-docs": {
      "source": "mattpocock/skills",
      "sourceType": "github",
      "skillPath": "skills/engineering/grill-with-docs/SKILL.md",
      "computedHash": "96718e0a89e48c7d76cc9759c7094a82ff34eb89ddc46a30388063fa9812356f"
    },
    "grill-me": {
      "source": "mattpocock/skills",
      "sourceType": "github",
      "skillPath": "skills/productivity/grill-me/SKILL.md",
      "computedHash": "7edd436132b43ea2973710f0c28a9879c9b0910651eb89a991a7e98cb6dbbacb"
    }
  }
}
```

### 技能开发最佳实践

#### 1. 安全优先的设计
- 始终设置 `disable-model-invocation: true` 防止意外调用
- 使用 `allow_implicit_invocation: false` 确保显式调用
- 实施适当的权限控制和验证机制

#### 2. 标准化配置结构
```yaml
interface:
  display_name: "技能显示名称"
  short_description: "简短描述"
policy:
  allow_implicit_invocation: false
  # 其他安全策略...
```

#### 3. 文档驱动开发
- SKILL.md 文件应包含完整的frontmatter和详细说明
- 提供清晰的使用示例和最佳实践
- 维护参考文档和示例代码

#### 4. 版本控制和哈希验证
- 使用 skills-lock.json 锁定技能版本
- 通过 computedHash 确保技能完整性
- 支持从GitHub等源管理器安装

### 技能集成流程

```mermaid
sequenceDiagram
participant Dev as "开发者"
participant SkillMgr as "技能管理器"
participant Repo as "远程仓库"
participant Project as "项目"
Dev->>SkillMgr : 安装新技能
SkillMgr->>Repo : 下载SKILL.md和配置
Repo-->>SkillMgr : 返回技能文件
SkillMgr->>Project : 验证技能完整性
SkillMgr->>Project : 更新skills-lock.json
Project-->>Dev : 技能安装完成
```

**图表来源**
- [skills-lock.json:166-171](file://skills-lock.json#L166-L171)
- [.agents/skills/grill-with-docs/SKILL.md:1-8](file://.agents/skills/grill-with-docs/SKILL.md#L1-L8)

**章节来源**
- [.agents/skills/grill-with-docs/SKILL.md:1-8](file://.agents/skills/grill-with-docs/SKILL.md#L1-L8)
- [.agents/skills/grill-with-docs/agents/openai.yaml:1-6](file://.agents/skills/grill-with-docs/agents/openai.yaml#L1-L6)
- [.agents/skills/grilling/SKILL.md:1-13](file://.agents/skills/grilling/SKILL.md#L1-L13)
- [.agents/skills/grill-me/SKILL.md:1-8](file://.agents/skills/grill-me/SKILL.md#L1-L8)
- [skills-lock.json:166-171](file://skills-lock.json#L166-L171)

## 依赖关系分析
- AsrEngine 与 SherpaOnnxAsrEngine：接口与实现的解耦，便于替换不同推理后端。
- StorageDependencies：集中管理数据库与仓库，降低耦合度。
- HttpModelFileDownloader：独立于业务逻辑，仅关注下载协议与错误映射。
- Theme：通过 FThemeData 组合，避免硬编码样式。
- **AI技能系统**：通过skills-lock.json统一管理，支持版本控制和完整性验证。

```mermaid
graph LR
AsrEngine["AsrEngine(接口)"] --> Impl["SherpaOnnxAsrEngine(实现)"]
Impl --> Adapter["底层适配器"]
Impl --> Store["存储服务"]
Store --> Repo["Repositories"]
Repo --> DB["AppDatabase"]
Impl --> Downloader["HttpModelFileDownloader"]
Theme["FThemeData"] --> UI["UI 组件"]
Skills["AI技能系统"] --> Config["skills-lock.json"]
Config --> Policy["安全策略"]
Policy --> Security["显式调用控制"]
```

**图表来源**
- [lib/domain/ports/asr_engine.dart:133-174](file://lib/domain/ports/asr_engine.dart#L133-L174)
- [lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart:19-41](file://lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart#L19-L41)
- [.agents/skills/grill-with-docs/agents/openai.yaml:4-5](file://.agents/skills/grill-with-docs/agents/openai.yaml#L4-L5)
- [skills-lock.json:166-171](file://skills-lock.json#L166-L171)

**章节来源**
- [lib/domain/ports/asr_engine.dart:133-174](file://lib/domain/ports/asr_engine.dart#L133-L174)
- [lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart:19-41](file://lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart#L19-L41)
- [.agents/skills/grill-with-docs/agents/openai.yaml:4-5](file://.agents/skills/grill-with-docs/agents/openai.yaml#L4-L5)
- [skills-lock.json:166-171](file://skills-lock.json#L166-L171)

## 性能考量
- ASR 引擎：
  - 窗口大小与采样率对内存与 CPU 的影响需严格测试
  - 使用 metrics.realTimeFactor 评估实时性，结合 diagnostics 定位失败窗口
- 存储：
  - 合理索引与事务边界，避免大事务阻塞
  - 文件布局与持久化提交策略（原子写入、校验）
- 网络：
  - 启用 Range 头进行断点续传，减少重复下载
  - 合理设置超时与重试策略，避免雪崩
- **AI技能系统**：
  - 技能加载缓存，避免重复解析
  - 显式调用减少不必要的模型调用开销
  - 版本锁定确保稳定的性能表现

## 故障排查指南
- ASR 引擎异常：
  - 检查 initialize 阶段的风险检测与适配器初始化
  - 查看 lastErrorCode 与 diagnostics 列表定位失败窗口原因
- 下载失败：
  - 确认 URL 是否为 HTTPS（默认强制）
  - 检查 Range 头与服务器兼容性
  - 观察 progress 回调与 cancellation 行为
- 存储问题：
  - 检查 StartupRecoveryService 是否成功恢复
  - 验证文件布局路径与权限
- **AI技能系统问题**：
  - 检查 skills-lock.json 中的技能哈希值是否正确
  - 验证 openai.yaml 配置文件语法
  - 确认显式调用策略是否正确配置

**章节来源**
- [lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart:109-158](file://lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart#L109-L158)
- [lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart:468-499](file://lib/data/services/asr/sherpa_onnx/sherpa_onnx_asr_engine.dart#L468-L499)
- [lib/data/services/models/http_model_file_downloader.dart:17-76](file://lib/data/services/models/http_model_file_downloader.dart#L17-L76)
- [.agents/skills/grill-with-docs/agents/openai.yaml:4-5](file://.agents/skills/grill-with-docs/agents/openai.yaml#L4-L5)

## 结论
通过明确的接口抽象与分层设计，"会迹"项目在 ASR 引擎扩展、存储后端替换、第三方服务集成、UI 主题定制以及AI技能系统集成方面具备良好的可扩展性与可维护性。新增的AI技能系统通过标准化的架构模式和显式调用策略，进一步提升了项目的安全性和专业性。遵循本文档的实践建议，开发者可以安全、高效地扩展新功能，同时保持系统的稳定性与性能。

## 附录：扩展示例与最佳实践

### 新增 ASR 引擎步骤：
- 实现 AsrEngine 接口，遵循窗口与采样率约束
- 映射底层异常为 AppFailure，统一错误码
- 暴露事件流与指标，完善诊断信息
- 在工厂中注册新引擎，提供创建方法

### 自定义存储后端步骤：
- 定义 Repository/Port 接口，保持与现有一致
- 实现多后端（本地/云端），提供迁移与恢复
- 在 StorageDependencies 中装配并管理生命周期

### 第三方服务集成步骤：
- 封装 HTTP 客户端，统一超时、认证、错误处理
- 支持取消令牌与进度回调
- 编写单元测试覆盖常见失败场景

### UI 主题扩展步骤：
- 在 theme.dart 中扩展 colors/style/icons
- 通过 touch 标志位适配不同平台
- 使用 FThemeData 组合，避免硬编码样式

### **AI技能系统开发步骤：**
- 创建 SKILL.md 文件，包含完整的frontmatter和详细说明
- 配置 agents/openai.yaml，设置安全策略和API参数
- 在 skills-lock.json 中注册技能，确保版本锁定
- 实现显式命令调用，禁用隐式模型调用
- 提供完整的文档生成和领域建模功能

### **技能开发最佳实践：**
- 安全优先：始终禁用隐式调用，实施严格的权限控制
- 标准化：遵循统一的配置结构和文件格式
- 文档驱动：提供完整的使用说明和示例代码
- 版本管理：使用哈希验证确保技能完整性
- 模块化：将复杂功能拆分为独立的子技能

[本节为实践建议，无需列出具体文件来源]